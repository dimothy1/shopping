...
<!--            ><img :src="item" alt="" />-->
<!--            </swiper-slide>-->
<!--            &lt;!&ndash; <swiper-slide><img src="/imgs/detail/phone-2.jpg" alt=""></swiper-slide>-->
<!--              <swiper-slide><img src="/imgs/detail/phone-3.jpg" alt=""></swiper-slide>-->
<!--              <swiper-slide><img src="/imgs/detail/phone-4.jpg" alt=""></swiper-slide>   &ndash;&gt;-->
...
<!--          <p class="item-info">{{ product.subTitle }}</p>-->
          ...
          <div class="item-price" style="margin-top:100px">{{ product.price }}元</div>
          <div class="line"></div>
          <!-- <div class="item-addr">
            <i class="icon-loc"></i>
          ...
          <div class="btn-group" style="margin-top: 250px">
<!--            <el-button style="background-color:orange"><a href="javascript:;" type="primary" class="btn btn-large fl" @click="addCart"-->
<!--            >加入购物车</a-->
<!--            ></el-button>-->
<!--            <el-button><a href="javascript:;" class="btn btn-large btn-buy fl">立即购买</a></el-button>-->
           <el-button icon="el-icon-s-goods" class="my-button" @click="addCart">加入购物车</el-button>
              <el-button icon="el-icon-check" type="danger" class="my-button" >立即购买</el-button>
...
created() {
    this.getProductInfo();
    window.console.log(this.prourl)
  },
  methods: {
    getProductInfo() {
      request.get(`/commodity/${this.id}`).then((res) => {
        this.product = res.data;
        request.get(`/description/${this.id}`,{
          params:{
            itemId:this.id
          }
        }).then((res1)=>{
          this.prourl=res1.data[0].itemDesc
          console.log(this.prourl)
        })
        // this.albumPics = res.data.image
    ...
    addCart() {
      ...
      request.post("/order", {
            itemId: this.id,
            payment: this.product.price,
            userId: this.userId,
            itemName:this.product.title,
            itemSrc:this.product.image,
          })
          .then(() => {
